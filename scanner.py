...
def jquants_headers():
    key = JQUANTS_API_KEY.strip()
    print(f"[DEBUG] APIキー長: {len(key)}文字 / 先頭5文字: {key[:5] if len(key)>=5 else 'SHORT'}")
    return {"x-api-key": key}

def get_listed_stocks():
    url = "https://api.jquants.com/v2/equities/master"
    headers = jquants_headers()
    print(f"[DEBUG] リクエスト送信: {url}")
    res = requests.get(url, headers=headers)
    print(f"[DEBUG] レスポンス: {res.status_code} / {res.text[:200]}")
...
def ai_scoring(candidates, news, twitter):
    ...
    candidates_text = "\n".join([
        f"銘柄:{q.get('Code','?')} 終値:{q.get('Close','?')}円"
...
de
cat > ~/stock-scanner/scanner.py << 'ENDOFFILE'
import os
import time
import schedule
import requests
