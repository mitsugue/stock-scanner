...
class ReleaseIdentityTests(unittest.TestCase):
    def test_version_sources_are_independent_and_current(self):
        self.assertEqual("v13.5.55", identity.product_version())
        self.assertEqual("13.5.55", identity.backend_version())
        self.assertEqual("13.5.55", identity.frontend_version())
    ...
    def test_version_files_are_plain_public_metadata(self):
        ...
        self.assertEqual({"version": "13.5.55"}, backend)
        self.assertEqual("13.5.55", frontend["version"])
    ...
    def test_product_version_never_falls_back_to_component_version(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for content in (
                None,
                "not-json",
                '{"schemaVersion":"wrong","productVersion":"v13.5.55"}',
                '{"schemaVersion":"argus-product-version-v1",'
                '"productVersion":"13.5.55"}',
                '{"schemaVersion":"argus-product-version-v1",'
                '"productVersion":"v13.5.55","frontendVersion":"13.5.55"}',
            ):
                if content is None:
